...
export async function programLoadCommand(args: string[]) {
  ...
  if (!programId) {
    p.log.error("Usage: solforge program load <programId> [--file PATH | --endpoint URL]");
    return;
  }
  ...
  const s = p.spinner(); s.start("Loading program into LiteSVM...");
  try {
    const res = await fetch(url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      jsonrpc: "2.0", id: 1, method: "solforgeLoadProgram", params: [programId, base64]
    }) });
    const json = await res.json();
    if (json.error) throw new Error(json.error.message || "program load failed");
