...
export async function tokenCreateCommand(args: string[]) {
  ...
  const s = p.spinner(); s.start("Creating mint...");
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "solforgeCreateMint", params: [mint ?? null, Number(decimals), owner] }),
    });
    ...
    if (amountBase || uiAmount) {
      ...
      const base = amountBase ?? (uiAmount ? toBaseUnits(uiAmount, Number(decimals)) : undefined);
      const res2 = await fetch(url, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", id: 2, method: "solforgeCreateTokenAccount", params: [createdMint, owner, String(base)] }),
      });
      const json2 = await res2.json();
      if (json2.error) throw new Error(json2.error.message || "create token account failed");
      s.stop("Token created");
      console.log(JSON.stringify({ mint: json.result, account: json2.result }, null, 2));
      return;
    }
    s.stop("Mint created");
