...
export async function tokenAdoptAuthorityCommand(args: string[]) {
  ...
  const s = p.spinner(); s.start("Adopting faucet as mint authority...");
  try {
    const res = await fetch(url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "solforgeAdoptMintAuthority", params: [mint] }) });
    const json = await res.json();
    if (json.error) throw new Error(json.error.message || "adopt authority failed");
