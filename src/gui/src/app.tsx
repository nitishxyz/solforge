...
export function App() {
  ...
  const onAirdrop = useCallback(async (address: string, lamports: string) => {
    const result = await submitAirdrop({ address, lamports });
    await loadStatus();
    return result.signature;
  }, [loadStatus]);

  const onMint = useCallback(async (mint: string, owner: string, amountRaw: string) => {
  ...
  const handleCloneProgram = useCallback(async (payload: { programId: string; endpoint?: string; withAccounts: boolean; accountsLimit?: number }) => {
    await cloneProgram(payload);
    await loadPrograms();
  }, [loadPrograms]);

  const handleCloneToken = useCallback(async (payload: { mint: string; endpoint?: string; cloneAccounts: boolean; holders?: number; allAccounts?: boolean }) => {
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
        <header className="rounded-xl bg-slate-900/60 p-6 shadow-soft backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-slate-100">SolForge Dashboard</h1>
              <p className="text-sm text-slate-400">Manage your local LiteSVM instance.</p>
            </div>
        ...
        <AirdropMintForm tokens={tokens} onAirdrop={onAirdrop} onMint={onMint} />
        <StatusPanel status={status} loading={loadingStatus} onRefresh={loadStatus} />
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          />
          <TokensPanel tokens={tokens} loading={loadingTokens} onRefresh={loadTokens} onAdd={openTokenModal} />
        </div>
      </div>
