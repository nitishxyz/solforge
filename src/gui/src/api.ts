...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const message = errPayload?.error || response.statusText || "Request failed";
...
export const submitMint = (body: MintPayload) =>
  request<{ ok: boolean; signature?: string; mint: string; owner: string; amount: string } | Record<string, unknown>>("/api/mint", {
    method: "POST",
    body: JSON.stringify(body),
  });
