    ...
    <section className="rounded-xl bg-slate-900/60 p-6 shadow-soft backdrop-blur">
      ...
      {status ? (
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatusCard title="Slot" value={formatter.format(status.slot)} subtitle={`Raw: ${status.slotBigint}`} />
          <StatusCard
            title="Block height"
        ...
        <p className="mt-4 text-sm text-slate-400">Status unavailable. Start the RPC to populate data.</p>
      )}
      {status?.latestBlockhash ? (
        <p className="mt-4 truncate text-xs text-slate-500">
          Latest blockhash: <span className="text-slate-300">{status.latestBlockhash}</span>
        </p>
      ) : null}
    </section>
  );
