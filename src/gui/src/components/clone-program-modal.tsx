...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (payload: { programId: string; endpoint?: string; withAccounts: boolean; accountsLimit?: number }) => Promise<void>;
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit({
        ...
        withAccounts,
        accountsLimit: withAccounts && accountsLimit.trim() ? Number(accountsLimit) : undefined,
      });
      onClose();
      setProgramId("");
      setEndpoint("");
    ...
    >
      <div className="space-y-4">
        <label className="flex flex-col gap-2">
          <span className="text-xs uppercase tracking-wide text-slate-500">Program ID</span>
          <input
            value={programId}
            onChange={(event: ChangeEvent<HTMLInputElement>) => setProgramId(event.target.value)}
        ...
        <label className="flex items-center gap-2 text-xs text-slate-400">
          <input
            ...
            onChange={(event: ChangeEvent<HTMLInputElement>) => setWithAccounts(event.target.checked)}
        ...
        {withAccounts ? (
          <label className="flex flex-col gap-2">
            <span className="text-xs uppercase tracking-wide text-slate-500">Accounts limit</span>
            <input
              value={accountsLimit}
              onChange={(event: ChangeEvent<HTMLInputElement>) => setAccountsLimit(event.target.value)}
              placeholder="100"
              className="rounded-lg border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/40"
            />
          </label>
