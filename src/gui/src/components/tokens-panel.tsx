    ...
    <section className="rounded-xl bg-slate-900/60 p-6 shadow-soft backdrop-blur">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">SPL mints registered in LiteSVM.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {tokens.map((token) => (
                <tr key={token.mint} className="hover:bg-slate-900/80">
                  <td className="py-2 pr-4 font-mono text-xs text-cyan-200">{token.mint}</td>
                  ...
                  <td className="py-2 text-xs">{token.isInitialized ? "Initialized" : "Uninitialized"}</td>
                </tr>
              ))}
            </tbody>
          </table>
