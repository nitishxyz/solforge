...
const text = (value: string, status = 200) =>
  new Response(value, { status, headers: { ...CORS, "Content-Type": "text/plain" } });
...
const wrap = (handler: (req: Request) => Promise<unknown> | unknown) =>
  async (req: Request) => {
    try {
...
const readJson = async <T extends Record<string, unknown>>(req: Request): Promise<T> => {
  try {
...
const parseLamports = (payload: { lamports?: string | number; sol?: string | number }) => {
  ...
  if (payload.sol != null) {
    ...
    if (!Number.isFinite(sol) || sol <= 0) throw new HttpError(400, "sol must be positive");
...
export function startGuiServer(opts: GuiStartOptions = {}) {
  ...
  const callRpc = async (method: string, params: any[] = []) => {
    ...
    const response: JsonRpcResponse = await rpcServer.handleRequest({ jsonrpc: "2.0", id: `gui-${Date.now()}`, method, params });
    if (response.error) throw new HttpError(400, response.error.message ?? "RPC error", response.error);
  ...
  const routes = {
    ...
    "/api/status": { GET: wrap(() => callRpc("solforgeGetStatus")), OPTIONS: okOptions },
    "/api/programs": { GET: wrap(() => callRpc("solforgeListPrograms")), OPTIONS: okOptions },
    "/api/tokens": { GET: wrap(() => callRpc("solforgeListTokensDetailed")), OPTIONS: okOptions },
    "/api/airdrop": {
      POST: wrap(async (req) => {
        const body = await readJson<{ address?: string; lamports?: string | number; sol?: string | number }>(req);
        const address = typeof body.address === "string" ? body.address.trim() : "";
        if (!address) throw new HttpError(400, "address is required");
        const lamports = parseLamports(body);
        const signature = await callRpc("requestAirdrop", [address, Number(lamports)]);
    ...
    "/api/mint": {
      POST: wrap(async (req) => {
        const body = await readJson<{ mint?: string; owner?: string; amountRaw?: string }>(req);
        const mint = typeof body.mint === "string" ? body.mint.trim() : "";
        const owner = typeof body.owner === "string" ? body.owner.trim() : "";
        const amountRaw = typeof body.amountRaw === "string" ? body.amountRaw.trim() : "";
        if (!mint || !owner || !amountRaw) throw new HttpError(400, "mint, owner and amountRaw are required");
    ...
    "/api/clone/program": {
      POST: wrap(async (req) => {
        const body = await readJson<{ programId?: string; endpoint?: string; withAccounts?: boolean; accountsLimit?: number }>(req);
        ...
        const accountsLimit = typeof body.accountsLimit === "number" ? body.accountsLimit : undefined;
        const res = await callRpc("solforgeAdminCloneProgram", [programId, { endpoint, withAccounts, accountsLimit }]);
    ...
    "/api/clone/token": {
      POST: wrap(async (req) => {
        const body = await readJson<{ mint?: string; endpoint?: string; cloneAccounts?: boolean; holders?: number; allAccounts?: boolean }>(req);
        const mint = typeof body.mint === "string" ? body.mint.trim() : "";
        if (!mint) throw new HttpError(400, "mint is required");
        const endpoint = typeof body.endpoint === "string" && body.endpoint.trim() ? body.endpoint.trim() : undefined;
        const cloneAccounts = body.cloneAccounts === true;
        const holders = typeof body.holders === "number" ? body.holders : undefined;
        const allAccounts = body.allAccounts === true;
        const mintResult = await callRpc("solforgeAdminCloneTokenMint", [mint, { endpoint }]);
        ...
        if (cloneAccounts) {
          accountsResult = await callRpc("solforgeAdminCloneTokenAccounts", [mint, { endpoint, holders, allAccounts }]);
        }
        await recordTokenClone(mint);
        return { mint: mintResult, adoptedAuthority: true, accounts: accountsResult };
      }),
      OPTIONS: okOptions,
    },
  } as const;

  const server = serve({ port, hostname: host, routes, development: process.env.NODE_ENV !== "production" });
