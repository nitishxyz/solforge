...
export async function writeDefaultConfig(opts: { force?: boolean } = {}) {
  ...
  try {
    if (!opts.force) await Bun.file(p).text().then(() => { throw new Error("exists"); });
  } catch {}
  const dir = dirname(p);
  if (dir && dir !== ".") {
...
export function setConfigValue<T extends Record<string, any>>(cfg: T, path: string, value: any): T {
