...
export function createLiteSVMWebSocketServer(rpcServer: LiteSVMRpcServer, port: number = 8900) {
  let nextSubId = 1;
  ...
  const server: Server = Bun.serve({
    ...
    websocket: {
      ...
      message(ws, data) {
        try {
          const msg = JSON.parse(typeof data === "string" ? data : Buffer.from(data as ArrayBuffer).toString("utf8"));
          ...
          if (method === "signatureSubscribe") {
            ...
            if (status) {
              ws.send(JSON.stringify({
                ...
                method: "signatureNotification",
                params: {
                  result: { context: { slot: status.slot }, value: { err: status.err } },
                  subscription: subId
                }
              }));
              subs.delete(subId);
          ...
          if (method === "logsSubscribe" || method === "slotSubscribe" || method === "programSubscribe" || method === "blockSubscribe") {
            const subId = nextSubId++;
            ws.send(JSON.stringify({ jsonrpc: "2.0", id, result: subId }));
            return;
          }
