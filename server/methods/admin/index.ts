				...
				if (pinfo) {
					...
					if (base64) {
						...
						return context.createErrorResponse(
							...
							-32603,
							"Clone program failed",
							{ message: "ProgramData bytes missing", programId, endpoint },
						);
					}
				}
			}
...
export const solforgeMintTo: RpcMethodHandler = async (id, params, context) => {
	try {
		...
		try {
			...
			const rawKeys: any[] = Array.isArray(msgAny.staticAccountKeys)
		...
		try {
			...
			try {
				(vtx as any).serialize = () => Buffer.from(rawBase64, "base64");
			} catch {}
