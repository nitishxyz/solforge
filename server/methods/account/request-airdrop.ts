...
export const requestAirdrop: RpcMethodHandler = (id, params, context) => {
  ...
  try {
    ...
    try { recentBlockhash = context.svm.latestBlockhash(); } catch {}
    if (!recentBlockhash) {
    ...
    const nonce = new Uint8Array(8); crypto.getRandomValues(nonce);
    const memoIx = new TransactionInstruction({ keys: [], programId: memoProgramId, data: Buffer.from(`airdrop:${Buffer.from(nonce).toString("hex")}`) });
    ...
    const beforeTo = toIndex >= 0 ? preBalances[toIndex] : (() => { try { return Number(context.svm.getBalance(toPubkey)); } catch { return 0; } })();
    ...
    try {
      ...
      const maybeErr = typeof rawErrFun === "function" ? rawErrFun() : rawErrFun;
      if (maybeErr) {
        ...
        try {
          ...
          else if (typeof anyRes?.meta === "function") logsForErr = anyRes.meta()?.logs?.() ?? [];
        } catch {}
        console.warn("[requestAirdrop] transfer failed. err=", maybeErr, " logs=\n", logsForErr.join("\n"));
      }
    } catch {}

    let signature = tx.signatures[0] ? context.encodeBase58(tx.signatures[0]) : context.encodeBase58(new Uint8Array(64).fill(0));
    ...
    const afterTo = toIndex >= 0 ? postBalances[toIndex] : (() => { try { return Number(context.svm.getBalance(toPubkey)); } catch { return 0; } })();
    const expectedDelta = Number(BigInt(lamports));
    if (afterTo - beforeTo < expectedDelta) {
      // Retry once with fresh blockhash + memo
      try {
        const bh2 = new Uint8Array(32); crypto.getRandomValues(bh2);
        const rb2 = context.encodeBase58(bh2);
        const memoProgramId2 = new PublicKey("MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr");
        const nonce2 = new Uint8Array(8); crypto.getRandomValues(nonce2);
        const memoIx2 = new TransactionInstruction({ keys: [], programId: memoProgramId2, data: Buffer.from(`airdrop-retry:${Buffer.from(nonce2).toString("hex")}`) });
        const msg2 = new TransactionMessage({ payerKey: faucet.publicKey, recentBlockhash: rb2, instructions: [ix, memoIx2] }).compileToV0Message();
        const tx2 = new VersionedTransaction(msg2); tx2.sign([faucet]);
        const res2 = context.svm.sendTransaction(tx2);
        try { const e2 = (res2 as any).err?.() ?? (res2 as any).err; if (e2) console.warn("[requestAirdrop] retry failed:", e2); } catch {}
        signature = tx2.signatures[0] ? context.encodeBase58(tx2.signatures[0]) : signature;
        context.notifySignature(signature);
        postBalances = staticKeys.map((pk) => { try { return Number(context.svm.getBalance(pk)); } catch { return 0; } });
      } catch {}
    }
    ...
    context.recordTransaction(signature, tx, { logs, fee: 5000, blockTime: Math.floor(Date.now() / 1000), preBalances, postBalances, err: recErr });
