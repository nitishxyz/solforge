...
export const solforgeGetStatus: RpcMethodHandler = async (id, _params, context) => {
  try {
    ...
    try {
      faucetLamports = context.svm.getBalance(faucet.publicKey as PublicKey) ?? 0n;
    } catch {}
    ...
    return context.createErrorResponse(id, -32603, "Status unavailable", error?.message || String(error));
  }
};

export const solforgeListPrograms: RpcMethodHandler = async (id, _params, context) => {
  try {
    const programs = context.listPrograms ? context.listPrograms() : [];
    const detailed = programs.map((programId) => {
      try {
    ...
    return context.createErrorResponse(id, -32603, "List programs failed", error?.message || String(error));
  }
};

export const solforgeListTokensDetailed: RpcMethodHandler = async (id, _params, context) => {
  try {
    const mints = context.listMints ? context.listMints() : [];
    const detailed = mints.map((mint) => {
      try {
    ...
    return context.createErrorResponse(id, -32603, "List tokens failed", error?.message || String(error));
  }
};
