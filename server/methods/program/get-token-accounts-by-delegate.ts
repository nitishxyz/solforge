...
export const getTokenAccountsByDelegate: RpcMethodHandler = async (id, params, context) => {
  ...
  try {
    ...
    const programFilter = requestedProgramId === token2022Id ? token2022Id : classicId;
    ...
    for (const r of rows) {
      try {
        ...
        const programPk = programFilter === token2022Id ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
        const dec = unpackAccount(new PublicKey(r.address), { data: Buffer.from(acc.data), owner: new PublicKey(acc.owner) }, programPk);
        ...
        if ((config?.encoding || "jsonParsed") === "jsonParsed") {
          out.push({
            ...
            account: {
              ...
              data: { program: "spl-token", parsed: { type: "account", info: {} }, space: data.length }
            }
          });
        } else {
          out.push({
