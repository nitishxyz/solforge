...
export const getProgramAccounts: RpcMethodHandler = async (id, params, context) => {
  ...
  try {
    ...
    const dataSlice = cfg?.dataSlice as { offset: number; length: number } | undefined;
    ...
    try { rows = (await context.store?.getAccountsByOwner(programStr, limit)) || []; } catch {}
    ...
    for (const r of rows) {
      try {
        ...
        // dataSize filter
        if (filters.some(f => typeof f?.dataSize === "number") && !filters.every(f => f.dataSize == null || (acc.data?.length ?? 0) === f.dataSize)) continue;
        // memcmp filters
        ...
        for (const f of filters) {
          ...
          try { needle = context.decodeBase58(bytesStr); } catch { try { needle = Buffer.from(bytesStr, "base64"); } catch { needle = new Uint8Array(); } }
          for (let i = 0; i < needle.length; i++) { if (raw[off + i] !== needle[i]) { memcmpOk = false; break; } }
          if (!memcmpOk) break;
        }
        if (!memcmpOk) continue;

        if (encoding === "jsonParsed" && (programPk.equals(TOKEN_PROGRAM_ID) || programPk.equals(TOKEN_2022_PROGRAM_ID))) {
          ...
          try {
            ...
            if (mintAcc) {
              const mintProg = new PublicKey(mintAcc.owner).equals(TOKEN_2022_PROGRAM_ID) ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
              const mi = unpackMint(dec.mint, { data: Buffer.from(mintAcc.data), owner: new PublicKey(mintAcc.owner) }, mintProg);
              ...
              data: {
                ...
                parsed: {
                  ...
                  info: {
                    ...
                    delegatedAmount: { amount: (BigInt(dec.delegatedAmount?.toString?.() ?? dec.delegatedAmount ?? 0)).toString(), decimals, uiAmount: null, uiAmountString: "0" },
                    delegate: dec.delegate ? dec.delegate.toBase58() : null,
                    rentExemptReserve: dec.isNative ? { amount: BigInt(dec.rentExemptReserve?.toString?.() ?? dec.rentExemptReserve ?? 0).toString(), decimals: 9, uiAmount: null, uiAmountString: "0" } : null,
          ...
          if (dataSlice && Number.isFinite(dataSlice.offset) && Number.isFinite(dataSlice.length)) {
            bytes = bytes.subarray(dataSlice.offset, dataSlice.offset + dataSlice.length);
          }
          out.push({
            pubkey: r.address,
            account: {
