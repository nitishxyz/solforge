...
export const getTokenSupply: RpcMethodHandler = (id, params, context) => {
  ...
  try {
    ...
    const info = unpackMint(mint, { data: Buffer.from(acc.data), owner: ownerPk }, programPk);
    const supply = BigInt(info.supply?.toString?.() ?? info.supply ?? 0);
    const ui = Number(supply) / Math.pow(10, info.decimals);
    return context.createSuccessResponse(id, {
      context: { slot: Number(context.slot) },
      value: { amount: supply.toString(), decimals: info.decimals, uiAmount: ui, uiAmountString: ui.toString() }
    });
  } catch (e: any) {
