...
const DEFAULT_PATH = process.env.SOLFORGE_FAUCET_PATH || ".solforge/faucet.json";
// Default to 1,000,000 SOL so we never run out in local dev.
// Override with SOLFORGE_FAUCET_LAMPORTS if desired.
const DEFAULT_INITIAL_LAMPORTS = BigInt(process.env.SOLFORGE_FAUCET_LAMPORTS || "1000000000000000"); // 1e15 lamports = 1,000,000 SOL
...
export function fundFaucetIfNeeded(
  ...
  let bal = 0n;
  try { bal = svm.getBalance(faucet.publicKey as PublicKey) || 0n; } catch { bal = 0n; }
  ...
  while (remaining > 0n && safety < 1000) {
    ...
    try {
      ...
      try { svm.airdrop(feeder.publicKey as PublicKey, 1_000_000_000n); } catch { break; }
    }

    // Transfer from feeder -> faucet
    try {
      const tx = new Transaction();
      try { tx.recentBlockhash = svm.latestBlockhash(); } catch {}
      tx.add(SystemProgram.transfer({ fromPubkey: feeder.publicKey, toPubkey: faucet.publicKey as PublicKey, lamports: Number(mint) }));
      tx.sign(feeder);
      svm.sendTransaction(tx);
      remaining -= mint;
    } catch {
      // If transfer fails, continue with next feeder to avoid stalling
      continue;
    }
  }

  try { bal = svm.getBalance(faucet.publicKey as PublicKey) || 0n; } catch {}
  return bal;
}
