...
test("executeTransaction transfers SOL from vault to recipient", async () => {
  ...
  const vaultAirdrop = await connection.requestAirdrop(vaultV2, 1 * LAMPORTS_PER_SOL);
  await connection.confirmTransaction(vaultAirdrop, "confirmed");

  // Prepare recipient
  const recipient = Keypair.generate().publicKey;

  const beforeVault = await connection.getBalance(vaultV2, { commitment: "confirmed" });
  const beforeRecipient = await connection.getBalance(recipient, { commitment: "confirmed" });
  ...
  const afterVault = await connection.getBalance(vaultV2, { commitment: "confirmed" });
  const afterRecipient = await connection.getBalance(recipient, { commitment: "confirmed" });
