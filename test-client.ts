...
async function testLiteSVMRpc() {
  ...
  try {
    ...
    const { value: payerBalance } = await rpc.getBalance(
      payer.address,
      { commitment: "confirmed" }
    ).send();
    console.log("   Payer balance:", Number(payerBalance) / 1_000_000_000, "SOL\n");
    ...
    const { value: simulation } = await rpc.simulateTransaction(
      base64Tx,
      { encoding: "base64" }
    ).send();
    ...
    const { value: recipientBalance } = await rpc.getBalance(
      recipient.address,
      { commitment: "confirmed" }
    ).send();
    console.log("   Recipient balance:", Number(recipientBalance) / 1_000_000_000, "SOL\n");
