...
export function AgiSidebar() {
  ...
  useEffect(() => {
    if (configError) {
      ...
      setInitError('Cannot connect to AGI server. Make sure it\'s running on port 3456.');
    } else if (config) {
  ...
  const handleOpenInFullUI = useCallback(() => {
    if (activeSessionId) {
      const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:3456';
            ...
            <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-2xl transform flex-col border-l border-border bg-background shadow-2xl translate-x-0">
              ...
              <div className="flex flex-1 items-center justify-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">Loading AGI configuration...</p>
                </div>
              </div>
            </div>
          </>
          ...
          {initError ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
              <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">{initError}</p>
              </div>
