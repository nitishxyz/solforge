...
const SwipeableCards: React.FC<SwipeableCardsProps> = ({
  ...
  const handleSwipe = useCallback(
    async (direction: "left" | "right", card: SwipeableCard) => {
      ...
      try {
        ...
        // Move to next card before hiding loading overlay
        removeTopCard();
      } catch (error) {
    ...
    [onSwipeRight, onSwipeLeft, resetCardPosition, removeTopCard]
  );
  ...
  const getSecurityColor = (score?: number) => {
    ...
    if (score <= 20) return "success"; // 0-20 is excellent (green)
    if (score <= 40) return "warning"; // 21-40 is good (yellow)
    return "error"; // 41-100 is poor (red)
  };
                ...
                >
                  <Box
                    style={{
                      ...
                      height: "100%",
                      backgroundColor: "#3b82f6", // Blue color for bonding curve
                    }}
                  />
                </Box>
  ...
  return (
    <Box style={{ paddingHorizontal: 20, paddingVertical: 5 }}>
      ...
      {/* Action Buttons */}
      <Box direction="row" gap="lg" center mt="lg">
        {/* Skip Button */}
        <Button
          size="lg"
          mode="error"
        ...
        >
          <Icon icon={Feather} name="x" size={28} color="white" />
        </Button>
        ...
        >
          <Icon icon={Feather} name="check" size={28} color="white" />
        </Button>
      </Box>
    </Box>
  );
